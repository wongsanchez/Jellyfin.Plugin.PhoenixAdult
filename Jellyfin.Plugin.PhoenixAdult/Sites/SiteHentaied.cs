...
{
    public class SiteHentaied : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if ((searchTitle.StartsWith("1", StringComparison.OrdinalIgnoreCase)
            ...
            {
                var sceneURL = new Uri(Helper.GetSearchBaseURL(siteNum) + $"/1-girl-4-eggs");
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            result.Item.Name = sceneData.SelectSingleText("//h1[@class='left-top-part']");
            result.Item.AddStudio("Hentaied");


            // if (sceneDataJSON != null)
            // {
            //     var date = (string)sceneDataJSON["grpah"][5]["datePublished"];
            //     if (date != null)
            //     {
            ...
            // var genreNode = sceneData.SelectNodesSafe("(//div[@class='Cats'])/a");
